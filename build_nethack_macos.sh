...
# These symlinks allow existing paths like NetHack/ and lua/ to work
# Skip if directory already exists (proper submodule setup)
if [ ! -e "NetHack" ]; then
    echo "Creating NetHack symlink..."
    ln -sf . NetHack
fi
if [ ! -e "lua" ]; then
    echo "Creating lua symlink..."
...
SOURCES=(
    ...
    "src/ios_structured_log.c"     # Structured JSON logging (when NH_STRUCTURED_LOGGING=1)
...
if [ -f "patches/extern_export.patch" ]; then
    echo "  [1/10] Applying extern_export.patch (export declarations)..."
...
if [ -f "patches/dlb_export.patch" ]; then
    echo "  [2/10] Applying dlb_export.patch (DLB file system)..."
...
if [ -f "patches/save_restore_export.patch" ]; then
    echo "  [3/10] Applying save_restore_export.patch (save/restore system)..."
...
if [ -f "patches/ios_moveloop_exit.patch" ]; then
    echo "  [4/10] Applying ios_moveloop_exit.patch (clean game exit)..."
...
if [ -f "patches/botl_reset_guard.patch" ]; then
    echo "  [5/10] Applying botl_reset_guard.patch (status re-init fix)..."
...
if [ -f "patches/ios_syscf_skip.patch" ]; then
    echo "  [6/10] Applying ios_syscf_skip.patch (skip system config)..."
...
if [ -f "patches/nhlua_defensive_init.patch" ]; then
    echo "  [7/10] Applying nhlua_defensive_init.patch (Lua safety)..."
...
if [ -f "patches/ios_tutorial_skip.patch" ]; then
    echo "  [8/10] Applying ios_tutorial_skip.patch (skip tutorial)..."
...
if [ -f "patches/ios_relink_timers_deferred.patch" ]; then
    echo "  [10/11] Applying ios_relink_timers_deferred.patch (multi-level restore fix)..."
    (cd "$ORIGIN_DIR" && patch -p1 -N -s -f < ../patches/ios_relink_timers_deferred.patch 2>/dev/null) || true
fi

# 11. iOS u_init stub (u_init.c) - Backward-compat stub for role templates
if [ -f "patches/ios_u_init_stub.patch" ]; then
    if ! grep -q "ios_reset_role_inventory_templates" "$ORIGIN_DIR/src/u_init.c" 2>/dev/null; then
        echo "  [11/11] Applying ios_u_init_stub.patch (u_init backward-compat stub)..."
        (cd "$ORIGIN_DIR" && patch -p1 -N -s -f < ../patches/ios_u_init_stub.patch 2>/dev/null) || true
    else
        echo "  [11/11] ios_u_init_stub.patch already applied (skipping)"
    fi
fi

echo "✓ All iOS patches applied (11 total)"
echo "  - 3 export patches (extern.h, dlb.h, save/restore)"
echo "  - 3 iOS adaptations (syscf, tutorial, moveloop)"
echo "  - 4 stability fixes (botl guard, Lua defensive, timer relink, u_init stub)"
